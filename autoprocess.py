...
def open_slice(inp, opt_root, threshold, min_length, min_interval, hop_size, max_sil_kept, _max, alpha, n_parts):
    ...
    if (ps_slice == []):
        for i_part in range(n_parts):
            cmd = '"%s" tools/new_slice_audio.py "%s" "%s" %s %s %s %s %s %s %s %s %s''' % (
...
def get_tts_wav(ref_wav_path, prompt_text, prompt_language, text, text_language, how_to_cut, top_k=20,
    ...
    if not ref_free:
        with torch.no_grad():
            ...
            print(f"ssl_content={ssl_content}")
            torch.save(ssl_content, f"test/{tag}/ssl_content.pth")

            codes = vq_model.extract_latent(ssl_content)
            prompt_semantic = codes[0, 0]
            prompt = prompt_semantic.unsqueeze(0).to(device)
            print(f"prompt={prompt}")
            torch.save(prompt, f"test/{tag}/prompt.pth")
    t1 = ttime()
    t.append(t1 - t0)
    ...
    if not ref_free:
        ...
        df=pd.DataFrame(phones1)
        df.to_csv(f"test/{tag}/phones1.csv",index=False)
    for i_text, text in enumerate(texts):
        ...
        print(f"phones2={phones2}")
        df = pd.DataFrame(phones2)
        df.to_csv(f"test/{tag}/phones2.csv", index=False,header=False)
        ...
        if (i_text in cache and if_freeze == True):
            ...
            with torch.no_grad():
                ...
                torch.save(pred_semantic, f"test/{tag}/pred_semantic.pth")
                print(f"idx={idx}")
                with open(f'test/{tag}/idx.txt', 'w') as file:
                    file.write(str(idx))
                pred_semantic = pred_semantic[:, -idx:].unsqueeze(0)
                torch.save(pred_semantic, f"test/{tag}/pred_semantic_unsqueeze.pth")
        ...
        print(refers)
        refer_pd=refers[0]
        torch.save(refer_pd,f"test/{tag}/refer_pd.pth")
